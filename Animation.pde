class Animation { //<>//
  ...
  void TriggerArray() {
    ...
    // soo this here contructs new trigger
    for (int y = 1; y < Variables; y++) {
      for (int x = 0; x < Triggers; x++) {
       if (Matrix[x][y] == true) {
          start = x;
          ...
          Trigger newTrigger;
          newTrigger = new Trigger(start, intervals, y);
...
class Trigger {
  ...
  Trigger(int start, int intervals, int parameter) {
    Start = start;
  ...
  void anit() {
    Ani.to(layer_1.gear0, duration,Parameters[ParameterID], value, easings[int(Easing.getValue())]);
    //println(value);
  }
}